...
class Agent:
    ...
    def get_action(self, state):
        ...
        available_p = [self.states[state][action]
...
class MDP:
    ...
    def add_state(self, name, action_reward_map=None, _pass=None):
        ...
        if _pass is None:
            self.states[name] = {
                '_value': 0.0, '_terminal': True, 'action_reward_map': None, '_pass': None}
            return

        self.states[name] = {'_value': 0.0, '_terminal': False,
                             '_action_reward_map': action_reward_map, '_pass': _pass}
    ...
    def sample(self, start_state_name, agent):
        ...
        while not self.states[state_name]['_terminal']:
            ...
            # print(action)
            if action not in state['_action_reward_map']:
                raise ValueError('invalid action')
            if action not in state['_pass']:
                raise ValueError('invalid action')

            available_states = [
                name for name in state['_pass'][action]]
            # print(available_states)
            available_pss = [state['_pass']
                             [action][name] for name in available_states]
            # print(available_pss)
            next_state = np.random.choice(available_states, p=available_pss)
            # print(next_state)

            episode.append(
                (state_name, action, state['_action_reward_map'][action], next_state))
...
if __name__ == '__main__':
    ...
    mdp_student.add_state('Facebook', action_reward_map={'Facebook': -1.0, 'Quit': 0.0}, _pass={
                          'Facebook': {'Facebook': 1.0}, 'Quit': {'Class1': 1.0}})
    mdp_student.add_state('Class1', action_reward_map={'Facebook': -1.0, 'Study': -2.0}, _pass={
                          'Facebook': {'Facebook': 1.0}, 'Study': {'Class2': 1.0}})
    mdp_student.add_state('Class2', action_reward_map={
                          'Sleep': 0.0, 'Study': -2.0}, _pass={'Sleep': {'Sleep': 1.0}, 'Study': {'Class3': 1.0}})
    mdp_student.add_state('Class3', action_reward_map={'Study': 10.0, 'Pub': 1.0}, _pass={
                          'Study': {'Sleep': 1.0}, 'Pub': {'Class1': 0.2, 'Class2': 0.4, 'Class3': 0.4}})
